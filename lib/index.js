  ...
  const handlers = {
    ...
    openTerminal: async (args) => {
      ...
      try {
        ...
        if (plat === 'win32') {
          tries.push(
            { name: 'Windows Terminal', run: () => spawn('wt.exe', ['-d', abs], { detached: true, stdio: 'ignore' }) },
            { name: 'PowerShell', run: () => spawn('powershell.exe', ['-NoExit', '-Command', "Set-Location -LiteralPath '" + String(abs).replace(/'/g, "''") + "'"], { detached: true, stdio: 'ignore' }) },
            { name: 'cmd', run: () => spawn('cmd.exe', ['/k', 'cd /d "' + String(abs).replace(/"/g, '""') + '"'], { detached: true, stdio: 'ignore' }) },
          )
        } else if (plat === 'darwin') {
          ...
          tries.push(
            ...
            { name: 'x-terminal-emulator', run: () => spawn('x-terminal-emulator', ['-e', 'bash', '-lc', 'cd "' + String(abs).replace(/"/g, '\\"') + '" && exec bash'], { detached: true, stdio: 'ignore' }) },
            { name: 'xterm', run: () => spawn('xterm', ['-e', 'bash', '-lc', 'cd "' + String(abs).replace(/"/g, '\\"') + '" && exec bash'], { detached: true, stdio: 'ignore' }) },
          )
        }
